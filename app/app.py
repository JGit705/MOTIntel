...
def _profile(make, model, age, n, rate, defects, mileage) -> VehicleProfile:
...
if not llm.credentials_available():
    st.info("AI summary unavailable — no Gemini API key configured. "
            "The figures below are unaffected.")
elif existing:
    st.write(existing)
    st.caption("Generated by Gemini from the retrieved figures on this page "
               "only. No other vehicle information is used. "
               "Retrieved from cache — no API call made.")
elif st.button("Explain what this means", type="primary"):
    with st.spinner("Reading the defect data ..."):
        summary = llm.summarise(profile)
    if summary:
        st.write(summary)
        st.caption("Generated by Gemini from the retrieved figures on this "
    ...
    st.caption("One API call, roughly 600 tokens. Summaries already generated "
               "for a vehicle are shown automatically and cost nothing.")
